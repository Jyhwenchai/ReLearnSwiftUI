  ...
  var body: some View {
    ScrollView {
      VStack(alignment: .leading, spacing: 20) {
        ...
        // MARK: - AttributedString 和 Markdown
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("长按此文本即可选择和复制。这个功能在需要用户复制内容时非常有用，比如邮箱地址、电话号码、代码片段等。选择功能可以通过 textSelection 修饰符来控制启用或禁用。")
              .textSelection(isTextSelectable ? .enabled : .disabled)
              .padding(8)
              .background(Color.blue.opacity(0.1))
              .cornerRadius(6)
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 8) {
            ...
            Text("这段文本使用系统动态类型设置，会跟随用户在系统设置中的字体大小偏好进行调整。")
              .padding(8)
              .background(Color.orange.opacity(0.1))
              .cornerRadius(6)
          }
        ...
        VStack(alignment: .leading, spacing: 12) {
          ...
          VStack(alignment: .leading, spacing: 8) {
            ...
            // 内容
            let messageContent = Text("您的账户余额不足。当前余额：")
              .foregroundColor(.primary) +
            Text("$12.34")
              .foregroundColor(.red)
              .bold() +
