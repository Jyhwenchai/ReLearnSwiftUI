...
struct TextExampleView03: View {
  ...
  private let longText =
    "这是一段很长的文本内容，用于演示多行文本的各种布局和显示效果。SwiftUI 的 Text 组件提供了丰富的选项来控制文本的显示方式，包括行数限制、截断模式、对齐方式和行间距等。通过这些修饰符，我们可以创建出美观且易读的用户界面。"

  private let englishLongText =
    "This is a long English text content used to demonstrate various layout and display effects of multi-line text. SwiftUI's Text component provides rich options to control text display, including line limits, truncation modes, alignment, and line spacing. With these modifiers, we can create beautiful and readable user interfaces."
