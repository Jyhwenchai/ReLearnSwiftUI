...
public struct ButtonExampleView02: View {
      ...
      ExampleSection("内置按钮样式完整展示") {
        VStack(spacing: 12) {
          ...
          Button("Automatic 样式") { }
            ...
            .modifier(StyleDescriptionModifier(
              name: "automatic",
      ...
      ExampleSection("动态样式调试器") {
        VStack(spacing: 16) {
          ...
          VStack {
            ...
            Button("动态预览按钮") { }
              .buttonStyle(getButtonStyle(selectedStyle))
  ...
  /// 根据字符串获取按钮样式
  private func getButtonStyle(_ style: String) -> some PrimitiveButtonStyle {
    switch style {
    case "automatic":
      return .automatic
    case "bordered":
